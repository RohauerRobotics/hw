...
w1 = 3
w2 = 100
Tfinal = 0.5 + (4*math.pi)/(w1)                                  # [s] length of time to run
Ts = 0.005                                    # [s] *approximate* sample period
###################################################
# run the experiment!
...
while True: # infinite loop until break statement
    ...
    # send out voltage
    V_A0 = math.sin(w1*t) + 0.1*math.sin(w2*t) + 1.5 # Sinusoidal output voltage amplitude = 1 
    A0_V = set_voltage(A0_pin, V_A0)
